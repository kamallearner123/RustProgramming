...
fn main(){
    ...
    match args.file2 {
        Some(file_path) => {
            let content = fs::read_to_string(&file_path).expect("File2 is not existing!!!");
            let count = content.lines().filter(|x| x.contains("main")).count();
            println!("Second file is {}, main count = {}", content, count);
        }
        None => println!("Second file is not mentioned!!!")
    }
}
